...
from src.modules import model_loader, llm_interaction, text_input
from src.conversation_manager import ConversationManager

def main():
    """
    Main function to run the console-based chat application.
    """
    print("--- Mind Vault Companion (Console Mode) ---")
    print("Initializing... This may take a while as the model is loaded.")

    try:
        model, processor, tokenizer = model_loader.load_model()
    ...
    while True:
        try:
            ...
            # 1. Add user turn. The manager will handle the system prompt on the first turn.
            user_content_list = [text_input.create_text_content(user_text)]
            manager.add_user_turn(user_content_list)

            # 2. Get response from LLM
            print("\nCompanion: Thinking...")
            assistant_response = llm_interaction.get_gemma_multimodal_response(
                model=model,
                processor=processor,
                tokenizer=tokenizer,
                messages=manager.get_history(), # Pass the full, correct history
                max_new_tokens=512
            )
